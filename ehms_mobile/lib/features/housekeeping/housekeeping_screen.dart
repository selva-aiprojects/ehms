...
class _HousekeepingScreenState extends ConsumerState<HousekeepingScreen>
  ...
  Widget _buildContent() {
    ...
    final inProgressCount = _tasks
        .where((t) => t['status'] == 'in_progress' || t['status'] == 'assigned')
        .length;
    final completedCount = _tasks
        .where((t) => t['status'] == 'completed')
        .length;
    final criticalCount = _tasks
        .where((t) => t['priority'] == 'critical' && t['status'] != 'completed')
        .length;
        ...
        child: Column(
          ...
          children: [
            Icon(
              ...
              size: 48,
              color: HmsColors.success.withValues(alpha: 0.3),
            ),
            ...
            child: Column(
              ...
              children: [
                ...
                _detailRow(
                  ...
                  priority,
                  Icons.flag_rounded,
                  priority == 'critical' ? HmsColors.danger : HmsColors.warning,
                ),
                if (notes.isNotEmpty) ...[
                  const SizedBox(height: 12),
                  Text(
        ...
        Flexible(
          child: Text(
            ...
            style: TextStyle(
              fontSize: 13,
              fontWeight: FontWeight.w600,
              color: HmsColors.textPrimary(context),
            ),
          ),
        ),
      ],
