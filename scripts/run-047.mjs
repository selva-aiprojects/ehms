/**
 * run-047.mjs — Apply 047_ticket_numeric_ids.sql
 * Adds ticket_number to support_tickets (public), guest_requests (tenant schemas),
 * and creates sequences for maintenance_tickets.
 * Usage: node scripts/run-047.mjs
 */
import pg from "pg";
...
async function run() {
  ...
  try {
    const sqlPath = join(__dirname, "../database", "047_ticket_numeric_ids.sql");
    const content = readFileSync(sqlPath, "utf-8");
    const statements = splitStatements(content);
    console.log(`▶ 047_ticket_numeric_ids.sql (${statements.length} statements)`);
    ...
    const stCount = await client.query(`SELECT count(*) AS cnt FROM public.support_tickets WHERE ticket_number IS NOT NULL`);
    console.log(`\n✅ support_tickets with ticket_number: ${stCount.rows[0].cnt}`);

    // Verify sequences
    const seqs = await client.query(`
      SELECT sequencename, schemaname FROM pg_sequences
      WHERE sequencename LIKE '%ticket%' OR sequencename LIKE '%guest_request%'
      ORDER BY schemaname, sequencename
    `);
    console.log("Sequences created:");
    for (const s of seqs.rows) console.log(`  - ${s.schema_name}.${s.sequence_name}`);

    console.log("\n✅ 047 applied successfully!");
  } finally {
    client.release();
    await pool.end();
  }
