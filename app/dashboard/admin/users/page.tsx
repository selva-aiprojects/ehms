  ...
  const userColumns = [
    { key: "name", header: "Name", render: (u: any) => (
      <div className="flex items-center gap-2">
        <div className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold text-white shrink-0" style={{ background: "var(--color-sidebar)" }}>
          {(u.first_name || "U").charAt(0)}
        </div>
        <div>
          <div className="font-medium text-sm" style={{ color: "var(--color-text)" }}>{u.first_name} {u.last_name || ""}</div>
          <div className="text-[10px]" style={{ color: "var(--color-text-faint)" }}>ID: {u.id?.slice(0, 8) || "—"}</div>
        </div>
      </div>
    )},
    ...
    { key: "scope", header: "Workspace Scope", render: (u: any) => {
      ...
      return (
        <span className="text-xs" style={{ color: propId ? "var(--color-navy)" : "var(--color-text-muted)" }}>
          {propName || (propId ? "Specific" : "Global")}
        </span>
      );
    }},
    { key: "last_login", header: "Last Login", render: (u: any) => {
      if (!u.last_login) return <span className="text-xs" style={{ color: "var(--color-text-faint)" }}>Never</span>;
      ...
      return <span className="text-xs" style={{ color: hours < 24 ? "var(--color-primary)" : "var(--color-text-muted)" }}>{display}</span>;
    }},
    { key: "status", header: "Status", render: (u: any) => (
      <div className="flex items-center gap-2">
        ...
        >
          {u.is_active !== false ? (
            <ToggleRight className="w-4 h-4" style={{ color: "var(--color-primary)" }} />
          ) : (
            <ToggleLeft className="w-4 h-4" style={{ color: "var(--color-text-faint)" }} />
          )}
        </button>
      </div>
    )},
    { key: "actions", header: "Actions", render: (u: any) => (
      <div className="flex items-center gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); handleExpandRow(u); }}
          ...
          {expandedUserId === u.id ? (
            <ChevronDown className="w-3.5 h-3.5" style={{ color: "var(--color-navy)" }} />
          ) : (
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-xl font-bold" style={{ color: "var(--color-navy)" }}>User Management</h1>
          <p className="text-sm mt-0.5" style={{ color: "var(--color-text-muted)" }}>System users, roles, permissions, and activity tracking</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="secondary" size="sm" onClick={() => setShowAddModal(true)}>
            <UserPlus className="w-3.5 h-3.5" /> Add User
          </Button>
          <button onClick={() => mutateUsers()} className="p-1.5 rounded-lg transition-colors" style={{ color: "var(--color-text-muted)" }} aria-label="Refresh">
      ...
      {activeTab === "users" && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="p-4 rounded-xl" style={{ background: "var(--color-light)", border: "1px solid var(--color-border)" }}>
              <div className="flex items-center gap-2 mb-1">
                <Users className="w-4 h-4" style={{ color: "var(--color-navy)" }} />
                <span className="text-xs font-semibold" style={{ color: "var(--color-text-muted)" }}>Total Users</span>
              </div>
              <span className="text-2xl font-bold" style={{ color: "var(--color-navy)" }}>{displayUsers.length}</span>
            </div>
            <div className="p-4 rounded-xl" style={{ background: "rgba(var(--color-primary-dark-rgb),0.08)", border: "1px solid rgba(var(--color-primary-dark-rgb),0.2)" }}>
              <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-2xl font-bold" style={{ color: "var(--color-primary)" }}>{activeUsers}</span>
            </div>
            <div className="p-4 rounded-xl" style={{ background: "rgba(var(--color-danger-rgb),0.06)", border: "1px solid rgba(var(--color-danger-rgb),0.15)" }}>
              <div className="flex items-center gap-2 mb-1">
                <X className="w-4 h-4" style={{ color: "var(--color-danger)" }} />
                <span className="text-xs font-semibold" style={{ color: "var(--color-text-muted)" }}>Inactive</span>
              </div>
              <span className="text-2xl font-bold" style={{ color: "var(--color-danger)" }}>{inactiveUsers}</span>
            </div>
            <div className="p-4 rounded-xl" style={{ background: "rgba(var(--color-warning-rgb),0.08)", border: "1px solid rgba(var(--color-warning-rgb),0.2)" }}>
              <div className="flex items-center gap-2 mb-1">
                <Clock className="w-4 h-4" style={{ color: "var(--color-warning)" }} />
                <span className="text-xs font-semibold" style={{ color: "var(--color-text-muted)" }}>Last 24h Logins</span>
              </div>
              <span className="text-2xl font-bold" style={{ color: "var(--color-warning)" }}>{last24hLogins}</span>
            </div>
          </div>
          ...
          <Card>
              ...
              action={
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)}
                    ...
                    style={{ borderColor: "var(--color-border)", color: roleFilter ? "var(--color-text)" : "var(--color-text-faint)" }}>
                  ...
                  <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}
                    className="px-2 py-1.5 text-xs rounded-lg border outline-none bg-white"
                    style={{ borderColor: "var(--color-border)", color: statusFilter ? "var(--color-text)" : "var(--color-text-faint)" }}>
                  ...
                  <select value={workspaceFilter} onChange={(e) => setWorkspaceFilter(e.target.value)}
                    ...
                    style={{ borderColor: "var(--color-border)", color: workspaceFilter ? "var(--color-text)" : "var(--color-text-faint)" }}>
            ...
            {loadingUsers ? (
              ...
              <div className="text-center py-8">
                <Users className="w-6 h-6 mx-auto mb-2" style={{ color: "var(--color-text-faint)" }} />
                <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>No users found matching your filters</p>
              </div>
            ) : (
              <div>
                <Table data={displayUsers} keyExtractor={(u: any) => u.id || Math.random().toString()} columns={userColumns} />
                {expandedUserId && (
                  <div className="border-t px-4 py-4" style={{ borderColor: "var(--color-border)", background: "var(--color-light)" }}>
                    {(() => {
                      ...
                      return (
                        <div className="space-y-3">
                          <div className="flex items-center gap-2">
                            <Activity className="w-4 h-4" style={{ color: "var(--color-navy)" }} />
                          ...
                          <div className="relative pl-6 space-y-0">
                            {loadingEvents ? (
                              <div className="flex justify-center py-4"><Loader2 className="w-4 h-4 animate-spin" style={{ color: "var(--color-text-faint)" }} /></div>
                            ) : (auditEvents || []).length === 0 ? (
                              <div className="text-xs py-2" style={{ color: "var(--color-text-faint)" }}>
                              ...
                              (auditEvents || []).slice(0, 8).map((e: any, i: number) => (
                                <div key={e.id || i} className="relative pb-3 last:pb-0">
                                  {i < Math.min((auditEvents || []).length, 8) - 1 && (
                                    <div className="absolute left-0 top-2 bottom-0 w-px" style={{ background: "var(--color-border)" }} />
                                  )}
                                  <div className="absolute left-[-5px] top-1.5 w-2.5 h-2.5 rounded-full border-2" style={{
                                    background: "var(--color-white)",
                                    borderColor: e.severity === "error" || e.severity === "critical" ? "var(--color-danger)" : e.severity === "warning" ? "var(--color-warning)" : "var(--color-primary)",
                                  }} />
                                  <div className="ml-4">
                                    <div className="text-xs font-medium" style={{ color: "var(--color-text)" }}>{e.title || e.action || e.event_type}</div>
                                    <div className="text-[10px] flex items-center gap-1 mt-0.5" style={{ color: "var(--color-text-faint)" }}>
        ...
        <Card>
          ...
          {loadingEvents ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>No audit events recorded yet</p>
            </div>
          ) : (
            <div className="space-y-0">
              {(auditEvents || []).map((e: any, i: number) => {
                const severityColor = e.severity === "error" || e.severity === "critical" ? "var(--color-danger)" : e.severity === "warning" ? "var(--color-warning)" : "var(--color-primary)";
                const severityBg = e.severity === "error" || e.severity === "critical" ? "rgba(var(--color-danger-rgb),0.08)" : e.severity === "warning" ? "rgba(var(--color-warning-rgb),0.08)" : "rgba(var(--color-primary-dark-rgb),0.08)";
                return (
                  <div key={e.id || i} className="flex items-start gap-3 py-3 px-1"
                    style={{ borderBottom: i < (auditEvents || []).length - 1 ? "1px solid var(--color-light)" : "none" }}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium" style={{ color: "var(--color-text)" }}>{e.title || e.event_type}</div>
                      <div className="text-xs mt-0.5" style={{ color: "var(--color-text-muted)" }}>{e.description || ""}</div>
                    </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={newUser.property_id}
                    ...
                    disabled={!!requesterPropertyId}
                    className="w-full px-3 py-2 rounded-lg border text-sm outline-none bg-white disabled:bg-slate-50 disabled:text-slate-400" style={{ borderColor: "var(--color-border)" }}>
      ...
      {showEditModal && editUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden">
            <div className="px-6 py-4 border-b flex items-center justify-between" style={{ borderColor: "var(--color-border)" }}>
              <h3 className="font-bold text-lg" style={{ color: "var(--color-navy)" }}>Edit User: {editUser.first_name} {editUser.last_name}</h3>
            ...
            <form onSubmit={handleUpdateUser} className="p-6 space-y-4">
              ...
              <div className="flex items-center justify-between p-3 rounded-lg" style={{ background: "var(--color-light)" }}>
                <div>
                  <span className="text-sm font-medium" style={{ color: "var(--color-text)" }}>Account Status</span>
                ...
                <button type="button" onClick={() => setEditUser({ ...editUser, is_active: !editUser.is_active })}
                  ...
                  {editUser.is_active ? (
                    ...
                    <ToggleLeft className="w-6 h-6" style={{ color: "var(--color-text-faint)" }} />
                  )}
                </button>
              </div>
              <div className="flex justify-end gap-2 pt-4 border-t" style={{ borderColor: "var(--color-border)" }}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
            <div className="px-6 py-5 text-center">
              <div className="w-12 h-12 rounded-full mx-auto mb-3 flex items-center justify-center" style={{ background: "rgba(var(--color-danger-rgb),0.1)" }}>
                <AlertCircle className="w-6 h-6" style={{ color: "var(--color-danger)" }} />
              </div>
              <h3 className="font-bold text-lg mb-1" style={{ color: "var(--color-navy)" }}>Delete User?</h3>
            ...
            <div className="px-6 pb-5 flex gap-2 justify-center">
              ...
              <Button variant="primary" size="sm" style={{ background: "var(--color-danger)" }} onClick={() => handleDeleteUser(showDeleteConfirm)} disabled={deleteUser.isMutating}>
