  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-xl font-bold" style={{ color: "var(--color-navy)" }}>Workspaces / Properties</h1>
          <p className="text-sm mt-0.5" style={{ color: "var(--color-text-muted)" }}>Manage all properties, buildings, floors across verticals</p>
        </div>
        ...
        <div className="flex items-center gap-1">
          {["", "hotel", "service_apartment", "rental_apartment", "workplace"].map((v) => (
            <button key={v || "all"} onClick={() => setFilterVertical(v)}
              ...
              style={{ background: filterVertical === v ? "var(--color-navy)" : "var(--color-light)", color: filterVertical === v ? "var(--color-white)" : "var(--color-text-muted)" }}>
              {v || "All"}
            </button>
          ))}
        </div>
      ...
      {isLoading && !properties.length ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>No properties found. Create your first workspace.</p>
        </div>
      ) : (
          ...
          {filteredProperties.map((p: any) => {
            ...
            return (
              <Card key={p.id} className="hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: "rgba(var(--color-navy-rgb),0.1)" }}>
                      <VertIcon className="w-5 h-5" style={{ color: "var(--color-navy)" }} />
                    </div>
                    <div>
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        ...
                        <code className="text-[10px] font-mono" style={{ color: "var(--color-text-faint)" }}>{p.code}</code>
                      </div>
                    </div>
                  </div>
                ...
                <div className="mt-3 space-y-1.5 text-xs" style={{ color: "var(--color-text-muted)" }}>
                  {p.city && (
                  ...
                  {p.star_rating && (
                    <div className="flex items-center gap-1.5">
                      ...
                      {Array.from({ length: p.star_rating }).map((_, i) => (
                        <span key={i} className="text-[10px]" style={{ color: "var(--color-warning)" }}>&#9733;</span>
                      ))}
                    </div>
                  )}
                </div>

                <div className="mt-3 pt-3 flex items-center justify-between" style={{ borderTop: "1px solid var(--color-light)" }}>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold mb-1" style={{ color: "var(--color-text)" }}>Property Name *</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold mb-1" style={{ color: "var(--color-text)" }}>Vertical Type *</label>
                ...
                <div>
                  <label className="block text-xs font-semibold mb-1" style={{ color: "var(--color-text)" }}>Booking Model *</label>
              ...
              <div>
                <label className="block text-xs font-semibold mb-1" style={{ color: "var(--color-text)" }}>Address</label>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold mb-1" style={{ color: "var(--color-text)" }}>Star Rating</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold mb-1" style={{ color: "var(--color-text)" }}>Check-in Time</label>
                  <input type="time" value={formData.check_in_time}
                    onChange={(e) => setFormData({ ...formData, check_in_time: e.target.value })}
                    className="w-full px-3 py-2 rounded-lg border text-sm outline-none" style={{ borderColor: "var(--color-border)" }} />
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1" style={{ color: "var(--color-text)" }}>Check-out Time</label>
              ...
              <div>
                <button type="button" onClick={() => setShowConfig(!showConfig)}
                  ...
                  style={{ background: showConfig ? "rgba(var(--color-navy-rgb),0.08)" : "var(--color-light)", color: "var(--color-navy)", border: "1px solid var(--color-border)" }}>
                  <Settings className="w-3.5 h-3.5" />
                  ...
                  <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {Object.entries(featureConfig).map(([key, feat]) => (
                      <button key={key} type="button" onClick={() => setFeatureConfig((prev) => ({ ...prev, [key]: { ...prev[key], enabled: !prev[key].enabled } }))}
                        ...
                        {feat.enabled ? <Eye className="w-3 h-3" style={{ color: "var(--color-primary)" }} /> : <EyeOff className="w-3 h-3" style={{ color: "var(--color-text-faint)" }} />}
                        <span style={{ color: feat.enabled ? "var(--color-text)" : "var(--color-text-muted)" }}>{feat.label}</span>
                      </button>
                    ))}
                  </div>
                )}
