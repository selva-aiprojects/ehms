  ...
  const roleColumns = [
    { key: "name", header: "Role", render: (r: any) => (
      <div className="flex items-center gap-2">
        <Shield className="w-4 h-4" style={{ color: "var(--color-navy)" }} />
        <span className="font-medium text-sm">{r.name.replace(/_/g, " ").replace(/\b\w/g, (c: string) => c.toUpperCase())}</span>
      </div>
    )},
    { key: "description", header: "Description", render: (r: any) => <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>{r.description || "—"}</span> },
      ...
      return (
        <div className="flex flex-wrap gap-1 max-w-xs">
          {(perms || []).slice(0, 3).map((p: string) => (
            <span key={p} className="text-[10px] px-1.5 py-0.5 rounded" style={{ background: "var(--color-border)", color: "var(--color-text)" }}>{p}</span>
          ))}
          {(perms?.length || 0) > 3 && <span className="text-[10px]" style={{ color: "var(--color-text-faint)" }}>+{perms!.length - 3} more</span>}
        </div>
      );
    }},
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold" style={{ color: "var(--color-navy)" }}>Roles & Permissions</h1>
          <p className="text-sm mt-0.5" style={{ color: "var(--color-text-muted)" }}>Manage system roles and their access permissions</p>
        </div>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b" style={{ borderColor: "var(--color-border)" }}>
                  <th className="text-left py-2.5 px-3 font-medium text-xs uppercase tracking-wider" style={{ color: "var(--color-text-muted)" }}>Module</th>
                  {displayRoles.filter((r: any) => r.is_system).map((r: any) => (
                    <th key={r.id} className="text-center py-2.5 px-2 font-medium text-[10px] uppercase tracking-wider" style={{ color: "var(--color-text-muted)" }}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={module} className="border-b" style={{ borderColor: "var(--color-light)" }}>
                    <td className="py-2.5 px-3 font-medium" style={{ color: "var(--color-text)" }}>{module}</td>
                    {displayRoles.filter((r: any) => r.is_system).map((r: any) => {
                      const perms = ROLE_PERMISSIONS[r.name] || [];
                      ...
                      return (
                        <td key={r.id} className="text-center py-2.5 px-2">
                          <div className="w-5 h-5 rounded-full mx-auto flex items-center justify-center"
                            style={{ background: hasAccess ? "rgba(var(--color-primary-dark-rgb),0.15)" : "rgba(var(--color-danger-rgb),0.08)" }}>
                            <div className="w-2 h-2 rounded-full"
                              style={{ background: hasAccess ? "var(--color-primary)" : "var(--color-danger)" }} />
                          </div>
                        </td>
                      );
                    })}
          ...
          {displayRoles.map((role: any) => {
            ...
            return (
              <Card key={role.id}>
                ...
                {roleUsers.length === 0 ? (
                  <div className="text-center py-4 text-sm" style={{ color: "var(--color-text-faint)" }}>No users assigned</div>
                ) : (
                  <div className="space-y-1">
                    {roleUsers.map((u: any) => (
                      <div key={u.id} className="flex items-center gap-2 py-1.5 px-1">
                        ...
                        <span className="text-sm" style={{ color: "var(--color-text)" }}>{u.first_name} {u.last_name || ""}</span>
                        <span className="text-xs ml-auto" style={{ color: "var(--color-text-muted)" }}>{u.email}</span>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div className="p-6 space-y-3 max-h-96 overflow-y-auto">
              {editPermissions.map((perm) => (
                <label key={perm} className="flex items-center gap-3 p-2 rounded-lg cursor-pointer hover:bg-slate-50">
                  ...
                  <span className="text-sm" style={{ color: "var(--color-text)" }}>{perm}</span>
                </label>
              ))}
            </div>
