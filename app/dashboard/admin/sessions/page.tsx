  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-xl font-bold" style={{ color: "var(--color-navy)" }}>Active Sessions</h1>
          <p className="text-sm mt-0.5" style={{ color: "var(--color-text-muted)" }}>Monitor active user sessions across the platform</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        {[
          { label: "Total Active", value: activeSessions.length.toString(), color: "var(--color-primary)", icon: Monitor },
          { label: "Desktop", value: (deviceTypes.desktop || deviceTypes.windows || deviceTypes.mac || 0).toString(), color: "var(--color-navy)", icon: Monitor },
          { label: "Mobile", value: (deviceTypes.mobile || deviceTypes.android || deviceTypes.ios || 0).toString(), color: "var(--color-warning)", icon: Smartphone },
          { label: "Web", value: (deviceTypes.web || deviceTypes.browser || 0).toString(), color: "var(--color-text-muted)", icon: Globe },
        ].map((s) => {
          const Icon = s.icon;
          return (
      ...
      <Card>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>Failed to load sessions</p>
          </div>
        ) : activeSessions.length === 0 ? (
          <div className="text-center py-8">
            <Monitor className="w-6 h-6 mx-auto mb-2" style={{ color: "var(--color-text-muted)" }} />
            <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>No active sessions</p>
          </div>
        ) : (
          <div className="space-y-1">
            {activeSessions.map((s: any) => {
              ...
              return (
                <div key={s.id} className="flex items-center justify-between p-3 rounded-lg" style={{ background: "var(--color-light)" }}>
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <DeviceIcon className="w-4 h-4 shrink-0" style={{ color: "var(--color-navy)" }} />
                    <div className="min-w-0">
                      <div className="text-sm font-medium truncate" style={{ color: "var(--color-text)" }}>
                        {s.user?.first_name} {s.user?.last_name || ""}
                      </div>
                      <div className="text-xs truncate" style={{ color: "var(--color-text-muted)" }}>{s.user?.email}</div>
                    </div>
                  </div>
                  <div className="hidden md:block text-xs" style={{ color: "var(--color-text-muted)" }}>
                    <div>IP: {s.ip_address || "—"}</div>
                    <div className="truncate max-w-[200px]">{s.user_agent?.slice(0, 60) || "—"}</div>
                  </div>
                  <div className="hidden lg:block text-xs text-right" style={{ color: "var(--color-text-faint)" }}>
