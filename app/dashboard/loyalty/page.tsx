  ...
  return (
    <div className="p-6 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-bold" style={{ color: "#1A3C5E" }}>Loyalty Program</h1>
          <p className="text-xs" style={{ color: "#64748B" }}>Manage tiers, points, and guest rewards</p>
        </div>
      </div>
        ...
        {tiers.length > 0 ? tiers.map((tier: any) => {
            ...
            <Card key={tier.id}>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h3 className="text-sm font-bold" style={{ color: "#1A3C5E" }}>{tier.name}</h3>
              ...
              <div className="mt-3 flex items-center gap-2">
                <span className="text-[10px] font-medium" style={{ color: "#94A3B8" }}>Points multiplier:</span>
          ...
          <Card className="col-span-3">
            <div className="py-8 text-center">
              <Trophy className="w-8 h-8 mx-auto mb-2" style={{ color: "#CBD5E1" }} />
              <p className="text-sm" style={{ color: "#94A3B8" }}>No loyalty tiers configured. Run the migration to seed default tiers.</p>
            </div>
          </Card>
        )}
      </div>
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <label className="text-xs font-medium block mb-1" style={{ color: "#64748B" }}>Guest</label>
            <select value={selectedGuest} onChange={(e) => setSelectedGuest(e.target.value)} className="w-full text-sm border rounded-lg px-3 py-2" style={{ borderColor: "#E2E8F0" }}>
          ...
          <div>
            <label className="text-xs font-medium block mb-1" style={{ color: "#64748B" }}>Points</label>
            <input type="number" value={points || ""} onChange={(e) => setPoints(parseInt(e.target.value) || 0)} className="w-full text-sm border rounded-lg px-3 py-2" style={{ borderColor: "#E2E8F0" }} placeholder="e.g. 500" />
          </div>
          <div>
            <label className="text-xs font-medium block mb-1" style={{ color: "#64748B" }}>Description</label>
            <input value={description} onChange={(e) => setDescription(e.target.value)} className="w-full text-sm border rounded-lg px-3 py-2" style={{ borderColor: "#E2E8F0" }} placeholder="e.g. Birthday bonus" />
          </div>
      ...
      <Card>
        ...
        {guests && guests.length > 0 ? (
          <div className="space-y-2">
            {guests
              ...
              .map((guest: any, i: number) => (
                <div key={guest.id} className="flex items-center gap-3 py-2 border-b" style={{ borderColor: "#F1F5F9" }}>
                  <span className="text-xs font-bold w-6 text-center" style={{ color: i < 3 ? "#F59E0B" : "#94A3B8" }}>
                    #{i + 1}
                  </span>
                  <div className="flex-1">
                    <p className="text-xs font-medium" style={{ color: "#1A3C5E" }}>{guest.first_name} {guest.last_name}</p>
                    <p className="text-[10px]" style={{ color: "#94A3B8" }}>{guest.total_stays || 0} stays</p>
                  </div>
          ...
          <p className="text-xs text-center py-8" style={{ color: "#94A3B8" }}>No loyalty members yet</p>
        )}
      </Card>
    </div>
  );
