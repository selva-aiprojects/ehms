  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-xl font-bold" style={{ color: "var(--color-navy)" }}>Accounts Receivable</h1>
          <p className="text-sm mt-0.5" style={{ color: "var(--color-text-muted)" }}>Customer invoices, receipts and collections</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Outstanding" value={formatCurrency(totalOutstanding)} sub="Across all invoices" color="var(--color-navy)" />
        <StatCard label="Overdue" value={formatCurrency(totalOverdue)} sub={invoices.filter((i) => i.status === "overdue").length + " overdue invoices"} color="var(--color-danger)" />
        <StatCard label="Collected MTD" value={formatCurrency(collectedMtd)} sub="Month to date collections" color="var(--color-primary)" />
        <StatCard label="Invoice Count" value={String(invoices.length)} sub={invoices.filter((i) => i.status === "paid").length + " paid"} color="var(--color-warning)" />
      </div>
      ...
      <Card>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-12">
            <Receipt className="w-8 h-8 mx-auto mb-3" style={{ color: "var(--color-border-strong)" }} />
            <p className="text-sm font-medium" style={{ color: "var(--color-text-muted)" }}>No invoices found</p>
            <p className="text-xs mt-1" style={{ color: "var(--color-text-faint)" }}>Try changing the status filter</p>
          </div>
        ) : (
          <Table
            data={filtered}
            ...
            columns={[
              { key: "invoice_number", header: "Invoice #", render: (inv: any) => <span className="font-mono text-xs font-medium" style={{ color: "var(--color-navy)" }}>{inv.invoice_number || inv.id || "—"}</span> },
              { key: "guest_name", header: "Guest / Customer", render: (inv: any) => <span className="text-sm">{inv.guest_name || inv.guest || "—"}</span> },
              { key: "property_name", header: "Property", render: (inv: any) => <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>{inv.property_name || "—"}</span> },
              { key: "grand_total", header: "Amount", render: (inv: any) => <span className="font-medium">₹{(inv.grand_total ?? 0).toLocaleString()}</span> },
              { key: "balance_due", header: "Balance Due", render: (inv: any) => <span style={{ color: (inv.balance_due ?? 0) > 0 ? "var(--color-danger)" : "var(--color-primary)" }}>{formatCurrency(inv.balance_due ?? 0)}</span> },
              { key: "due_date", header: "Due Date", render: (inv: any) => <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>{inv.due_date ? formatDate(inv.due_date) : "—"}</span> },
              ...
              { key: "days_overdue", header: "Days Overdue", render: (inv: any) => {
                ...
                return d > 0 ? <span className="text-xs font-medium" style={{ color: "var(--color-danger)" }}>{d}d</span> : <span className="text-xs" style={{ color: "var(--color-text-faint)" }}>—</span>;
              }},
            ]}
          />
        )}
