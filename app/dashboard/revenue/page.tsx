  ...
  if (isLoading || !revenue) {
    return (
      <div className="p-6 flex items-center justify-center" style={{ minHeight: "60vh" }}>
        ...
        <span className="ml-2 text-sm" style={{ color: "var(--color-text-muted)" }}>Loading revenue data...</span>
      </div>
    );
  }
    ...
    <div className="p-6 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold" style={{ color: "var(--color-navy)" }}>Revenue Dashboard</h1>
            <p className="text-xs" style={{ color: "var(--color-text-muted)" }}>Key performance indicators for the last 30 days</p>
          </div>
        </div>
        <select
          value={period}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <KPICard icon={Bed} label="Occupancy" value={`${occupancy.percentage}%`} subtext={`${occupancy.occupied}/${occupancy.total} rooms`} color="var(--color-info)" />
        <KPICard icon={DollarSign} label="Total Revenue" value={formatCurrency(rev.total)} color="var(--color-success)" />
        <KPICard icon={TrendingUp} label="ADR" value={formatCurrency(rev.adr)} subtext="Avg Daily Rate" color="#8B5CF6" />
        <KPICard icon={DollarSign} label="RevPAR" value={formatCurrency(rev.revpar)} subtext="Per Available Room" color="var(--color-warning)" />
        <KPICard icon={Users} label="Active Stays" value={String(bookings.activeStays)} color="#EC4899" />
        <KPICard icon={AlertTriangle} label="Cancellation Rate" value={`${bookings.cancellationRate}%`} subtext={`${bookings.cancellations} cancelled`} color="var(--color-danger)" trend={bookings.cancellationRate > 15 ? "down" : "up"} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card>
          ...
          {dailyTrend.length > 0 ? (
            <MiniBarChart data={dailyTrend} labelKey="date" valueKey="revenue" color="var(--color-info)" />
          ) : (
            <p className="text-xs text-center py-8" style={{ color: "var(--color-text-faint)" }}>No data available</p>
          )}
        </Card>
                  ...
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="h-2 rounded-full bg-gray-100 overflow-hidden">
                        <div className="h-full rounded-full" style={{ width: `${pct}%`, background: "var(--color-info)" }} />
                      </div>
                    </div>
                    <span className="text-xs font-medium" style={{ color: "var(--color-navy)" }}>{formatCurrency(Number(s.revenue))}</span>
                    <span className="text-[10px]" style={{ color: "var(--color-text-faint)" }}>{pct}%</span>
                  </div>
                );
              })}
            </div>
          ...
          {byRoomType.length > 0 ? (
            <div className="space-y-2">
              {byRoomType.map((r: any, i: number) => (
                <div key={i} className="flex items-center justify-between py-1.5 border-b" style={{ borderColor: "var(--color-light)" }}>
                  <span className="text-xs font-medium" style={{ color: "var(--color-navy)" }}>{r.room_type || "Room"}</span>
                  <div className="flex items-center gap-3">
                    <span className="text-[10px]" style={{ color: "var(--color-text-muted)" }}>{r.bookings} bookings</span>
                    <span className="text-xs font-semibold" style={{ color: "var(--color-navy)" }}>{formatCurrency(Number(r.revenue))}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          {upcomingArrivals.length > 0 ? (
            <div className="space-y-2">
              {upcomingArrivals.map((a: any, i: number) => (
                <div key={i} className="flex items-center gap-3 py-1.5 border-b" style={{ borderColor: "var(--color-light)" }}>
                  <CalendarCheck className="w-4 h-4 shrink-0" style={{ color: "var(--color-info)" }} />
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium truncate" style={{ color: "var(--color-navy)" }}>{a.guest_name}</p>
                  ...
                  <span className="text-xs font-medium" style={{ color: "var(--color-navy)" }}>{formatCurrency(Number(a.total_amount))}</span>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-xs text-center py-8" style={{ color: "var(--color-text-faint)" }}>No upcoming arrivals</p>
          )}
        </Card>
      </div>
    </div>
