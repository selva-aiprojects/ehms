      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2" style={{ color: "#1A3C5E" }}>
            <BookOpen className="w-6 h-6 text-[#2BAE8E]" /> Menu Management
          </h1>
          <p className="text-[#64748B] mt-1 text-sm">Manage restaurant menu items, pricing, and availability.</p>
        </div>
        <Button className="bg-[#2BAE8E] hover:bg-[#239B7E] text-white" onClick={openAdd}>
      ...
      <div className="flex flex-col md:flex-row gap-3 items-start md:items-center">
        ...
        <div className="relative">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
            className="pl-9 pr-3 py-1.5 border border-[#E2E8F0] rounded-lg text-sm focus:ring-2 focus:ring-[#2BAE8E] focus:outline-none w-48"
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
            {filtered.map((item: any) => (
              <div key={item.id} className="border border-[#E2E8F0] rounded-xl p-4 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-[#1A3C5E] truncate">{item.item_name}</span>
                      {item.is_veg ? (
                    ...
                    <div className="text-xs text-[#64748B] mt-0.5">{item.category}</div>
                    {item.description && <div className="text-xs text-[#64748B] mt-1 truncate">{item.description}</div>}
                  </div>
                  <div className="text-right ml-2">
                    <div className="font-bold text-[#2BAE8E]">₹{item.price}</div>
                    {item.prep_time_mins && <div className="text-[10px] text-[#64748B]">{item.prep_time_mins}m prep</div>}
                  </div>
                </div>
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-[#E2E8F0]">
                  <button onClick={() => handleToggle(item)}
                    className={`text-xs px-3 py-1 rounded-full font-medium transition-all ${item.is_available ? "bg-[#ECFDF5] text-[#065F46] hover:bg-[#D1FAE5]" : "bg-[#FEE2E2] text-[#991B1B] hover:bg-[#FECACA]"}`}>
                    {item.is_available ? "Available" : "Unavailable"}
                  </button>
                  <div className="flex items-center gap-1">
                    <button onClick={() => openEdit(item)} className="p-1.5 rounded-lg hover:bg-[#F5F7FA] text-[#64748B] hover:text-[#1A3C5E] transition-colors">
                      <Edit3 className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => handleDelete(item.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-[#64748B] hover:text-[#E53E3E] transition-colors">
        ...
        <>
          ...
          <div className="fixed inset-y-0 right-0 w-full max-w-md bg-white shadow-2xl z-50 animate-in slide-in-from-right duration-300 flex flex-col">
            <div className="px-6 py-4 border-b flex items-center justify-between">
              <h2 className="text-lg font-semibold text-[#1A3C5E]">{editingId ? "Edit Menu Item" : "Add Menu Item"}</h2>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600"><XCircle className="w-5 h-5" /></button>
            </div>
            <div className="p-6 flex-1 overflow-y-auto space-y-4">
              <div>
                <label className="block text-sm font-medium text-[#1A3C5E] mb-1">Item Name *</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-[#1A3C5E] mb-1">Price *</label>
                  <input type="number" min={0} value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })}
                    className="w-full border border-[#E2E8F0] rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#2BAE8E] focus:outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[#1A3C5E] mb-1">Category</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-[#1A3C5E] mb-1">Prep Time (min)</label>
                  <input type="number" min={0} value={form.prep_time_mins} onChange={(e) => setForm({ ...form, prep_time_mins: e.target.value })}
                    className="w-full border border-[#E2E8F0] rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#2BAE8E] focus:outline-none" />
                </div>
                <div className="flex items-end gap-4 pb-1">
                  <label className="flex items-center gap-2 text-sm text-[#1A3C5E] cursor-pointer">
              ...
              <div>
                <label className="block text-sm font-medium text-[#1A3C5E] mb-1">Photo URL</label>
            ...
            <div className="p-4 border-t bg-[#F5F7FA]">
              <button onClick={handleSave} disabled={saving || !form.item_name || !form.price}
                className="w-full bg-[#1A3C5E] hover:bg-[#132d47] text-white px-4 py-3 rounded-lg font-medium transition-colors flex items-center justify-center gap-2 disabled:opacity-50">
