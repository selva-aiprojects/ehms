...
export default function HRMastersPage() {
  ...
  const inputStyle: React.CSSProperties = { border: "1px solid var(--color-border)", borderRadius: "8px", padding: "8px 12px", width: "100%", fontSize: "13px", outline: "none" };
  const selectStyle: React.CSSProperties = { ...inputStyle, color: "var(--color-text)" };

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-xl font-bold" style={{ color: "var(--color-sidebar)" }}>HR Masters</h1>
          <p className="text-sm mt-0.5" style={{ color: "var(--color-text-muted)" }}>Manage holidays, overtime, attendance policies & document types</p>
        </div>
      </div>
      ...
      {/* Tabs */}
      <div className="flex items-center gap-1 p-1 rounded-lg" style={{ background: "var(--color-light)" }}>
        {TABS.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            <button
              ...
              style={{
                ...
                color: isActive ? "var(--color-sidebar)" : "var(--color-text-muted)",
        ...
        <Card>
          <CardHeader
            ...
            action={
              <div className="flex items-center gap-2">
                ...
                />
                <button onClick={() => hMutate()} className="p-1.5 rounded-lg transition-colors" style={{ color: "var(--color-text-muted)" }} aria-label="Refresh">
                  <RefreshCw className="w-4 h-4" />
                </button>
                <button onClick={() => { setShowHolidayForm(true); }} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-white transition-colors" style={{ background: "var(--color-sidebar)" }}>
          ...
          {hLoading ? (
            ...
            <div className="text-center py-8">
              <Sun className="w-6 h-6 mx-auto mb-2" style={{ color: "var(--color-text-muted)" }} />
              <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>No holidays found</p>
            </div>
          ) : (
            <Table
              data={holidayData}
              ...
              columns={[
                ...
                { key: "date", header: "Date", render: (h: any) => <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>{h.date ? new Date(h.date).toLocaleDateString() : "—"}</span> },
                { key: "type", header: "Type", render: (h: any) => <Badge variant={h.type === "public" ? "teal" : h.type === "optional" ? "amber" : "navy"}>{h.type}</Badge> },
                { key: "applicable_to", header: "Applicable To", render: (h: any) => <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>{h.applicable_to || "All"}</span> },
                { key: "actions", header: "Actions", render: () => <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>—</span> },
              ]}
            />
          )}
          {showHolidayForm && (
            <div className="mt-4 p-4 rounded-lg" style={{ background: "var(--color-light)", border: "1px solid var(--color-border)" }}>
              <h4 className="text-sm font-semibold mb-3" style={{ color: "var(--color-sidebar)" }}>New Holiday</h4>
              <div className="grid grid-cols-2 gap-3 mb-3">
                ...
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Applicable To</label>
              ...
              <div className="flex items-center justify-end gap-2">
                <button onClick={() => setShowHolidayForm(false)} className="px-3 py-1.5 rounded-lg text-xs font-medium" style={{ color: "var(--color-text-muted)", background: "var(--color-white)", border: "1px solid var(--color-border)" }}>Cancel</button>
                <button onClick={() => handleCreate("/api/hr/holidays", holidayForm, hMutate, setShowHolidayForm, setHSaving, "Holiday")} disabled={hSaving} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-white transition-colors" style={{ background: "var(--color-sidebar)" }}>
      ...
      {activeTab === "overtime" && (
        <Card>
          <CardHeader
            ...
            action={
              <div className="flex items-center gap-2">
                <button onClick={() => oMutate()} className="p-1.5 rounded-lg transition-colors" style={{ color: "var(--color-text-muted)" }} aria-label="Refresh">
                  <RefreshCw className="w-4 h-4" />
                </button>
                <button onClick={() => { setShowOvertimeForm(true); }} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-white transition-colors" style={{ background: "var(--color-sidebar)" }}>
          ...
          {oLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>No overtime policies found</p>
            </div>
          ) : (
            <Table
              data={overtimeData}
              ...
              columns={[
                ...
                { key: "min_hours", header: "Min Hours", render: (p: any) => <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>{p.min_hours}h</span> },
                { key: "max_hours_per_day", header: "Max Hours/Day", render: (p: any) => <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>{p.max_hours_per_day}h</span> },
                { key: "applicable_shifts", header: "Applicable Shifts", render: (p: any) => <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>{p.applicable_shifts || "All"}</span> },
                { key: "actions", header: "Actions", render: () => <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>—</span> },
              ]}
            />
          )}
          {showOvertimeForm && (
            <div className="mt-4 p-4 rounded-lg" style={{ background: "var(--color-light)", border: "1px solid var(--color-border)" }}>
              <h4 className="text-sm font-semibold mb-3" style={{ color: "var(--color-sidebar)" }}>New Overtime Policy</h4>
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Name</label>
                ...
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Multiplier</label>
                ...
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Min Hours</label>
                ...
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Max Hours / Day</label>
                ...
                <div className="col-span-2">
                  <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Applicable Shifts</label>
              ...
              <div className="flex items-center justify-end gap-2">
                <button onClick={() => setShowOvertimeForm(false)} className="px-3 py-1.5 rounded-lg text-xs font-medium" style={{ color: "var(--color-text-muted)", background: "var(--color-white)", border: "1px solid var(--color-border)" }}>Cancel</button>
                <button onClick={() => handleCreate("/api/hr/overtime-policies", overtimeForm, oMutate, setShowOvertimeForm, setOSaving, "Overtime policy")} disabled={oSaving} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-white transition-colors" style={{ background: "var(--color-sidebar)" }}>
      ...
      {activeTab === "attendance" && (
        <Card>
          <CardHeader
            ...
            action={
              <div className="flex items-center gap-2">
                <button onClick={() => aMutate()} className="p-1.5 rounded-lg transition-colors" style={{ color: "var(--color-text-muted)" }} aria-label="Refresh">
                  <RefreshCw className="w-4 h-4" />
                </button>
                <button onClick={() => { setShowAttendanceForm(true); }} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-white transition-colors" style={{ background: "var(--color-sidebar)" }}>
                  <Plus className="w-3.5 h-3.5" /> Add Policy
                </button>
              </div>
            }
          ...
          {aLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>No attendance policies found</p>
            </div>
          ) : (
            <Table
              data={attendanceData}
              ...
              columns={[
                { key: "name", header: "Name", render: (p: any) => <span className="text-sm font-medium">{p.name}</span> },
                { key: "late_threshold", header: "Late Threshold", render: (p: any) => <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>{p.late_threshold} min</span> },
                { key: "half_day_threshold", header: "Half Day", render: (p: any) => <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>{p.half_day_threshold} min</span> },
                { key: "grace_period", header: "Grace Period", render: (p: any) => <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>{p.grace_period} min</span> },
          ...
          {showAttendanceForm && (
            <div className="mt-4 p-4 rounded-lg" style={{ background: "var(--color-light)", border: "1px solid var(--color-border)" }}>
              <h4 className="text-sm font-semibold mb-3" style={{ color: "var(--color-sidebar)" }}>New Attendance Policy</h4>
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Name</label>
                ...
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Late Threshold (min)</label>
                ...
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Half Day Threshold (min)</label>
                ...
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Early Exit Threshold (min)</label>
                ...
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Grace Period (min)</label>
                ...
                <div className="flex items-end pb-2">
                  <label className="flex items-center gap-2 text-xs font-medium" style={{ color: "var(--color-text-muted)" }}>
                    <input type="checkbox" checked={attendanceForm.requires_geo} onChange={(e) => setAttendanceForm({ ...attendanceForm, requires_geo: e.target.checked })} style={{ width: "16px", height: "16px", accentColor: "var(--color-sidebar)" }} />
                    Requires Geo-Location
                  </label>
                </div>
              </div>
              <div className="flex items-center justify-end gap-2">
                <button onClick={() => setShowAttendanceForm(false)} className="px-3 py-1.5 rounded-lg text-xs font-medium" style={{ color: "var(--color-text-muted)", background: "var(--color-white)", border: "1px solid var(--color-border)" }}>Cancel</button>
                <button onClick={() => handleCreate("/api/hr/attendance-policies", attendanceForm, aMutate, setShowAttendanceForm, setASaving, "Attendance policy")} disabled={aSaving} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-white transition-colors" style={{ background: "var(--color-sidebar)" }}>
      ...
      {activeTab === "documents" && (
        <Card>
          <CardHeader
            ...
            action={
              <div className="flex items-center gap-2">
                <button onClick={() => dMutate()} className="p-1.5 rounded-lg transition-colors" style={{ color: "var(--color-text-muted)" }} aria-label="Refresh">
                  <RefreshCw className="w-4 h-4" />
                </button>
                <button onClick={() => { setShowDocForm(true); }} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-white transition-colors" style={{ background: "var(--color-sidebar)" }}>
          ...
          {dLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>No document types found</p>
            </div>
          ) : (
            <Table
              data={docData}
              ...
              columns={[
                ...
                { key: "code", header: "Code", render: (d: any) => <span className="font-mono text-xs" style={{ color: "var(--color-text-muted)" }}>{d.code || "—"}</span> },
          ...
          {showDocForm && (
            <div className="mt-4 p-4 rounded-lg" style={{ background: "var(--color-light)", border: "1px solid var(--color-border)" }}>
              <h4 className="text-sm font-semibold mb-3" style={{ color: "var(--color-sidebar)" }}>New Document Type</h4>
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Name</label>
                ...
                <div className="flex items-end pb-2">
                  <label className="flex items-center gap-2 text-xs font-medium" style={{ color: "var(--color-text-muted)" }}>
                    <input type="checkbox" checked={docForm.is_mandatory} onChange={(e) => setDocForm({ ...docForm, is_mandatory: e.target.checked })} style={{ width: "16px", height: "16px", accentColor: "var(--color-sidebar)" }} />
                    Is Mandatory
                  </label>
                </div>
                <div className="col-span-2">
              ...
              <div className="flex items-center justify-end gap-2">
                <button onClick={() => setShowDocForm(false)} className="px-3 py-1.5 rounded-lg text-xs font-medium" style={{ color: "var(--color-text-muted)", background: "var(--color-white)", border: "1px solid var(--color-border)" }}>Cancel</button>
                <button onClick={() => handleCreate("/api/hr/document-types", docForm, dMutate, setShowDocForm, setDSaving, "Document type")} disabled={dSaving} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-white transition-colors" style={{ background: "var(--color-sidebar)" }}>
