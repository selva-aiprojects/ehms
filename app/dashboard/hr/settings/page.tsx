...
function SkeletonRow() {
  return <div className="h-14 rounded animate-pulse mb-2" style={{ background: "var(--color-light)" }} />;
}
...
export default function HRSettingsPage() {
  ...
  const renderForm = () => {
    ...
    const common = (
      <div className="flex items-center justify-end gap-2 pt-3 mt-4" style={{ borderTop: "1px solid var(--color-border)" }}>
        <button onClick={() => { setShowForm(false); resetForm(); }} className="px-3 py-1.5 rounded-lg text-xs font-medium" style={{ color: "var(--color-text-muted)", background: "var(--color-light)" }}>Cancel</button>
        <button onClick={handleSave} disabled={saving} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-white transition-colors" style={{ background: "var(--color-primary)" }}>
    ...
    if (activeSection === "departments") return (
      <div className="space-y-3 text-sm">
        <div>
          <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Department Name *</label>
        ...
        <div>
          <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Manager</label>
    ...
    if (activeSection === "designations") return (
      <div className="space-y-3 text-sm">
        <div>
          <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Designation Name *</label>
        ...
        <div>
          <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Level</label>
    ...
    if (activeSection === "bands") return (
      <div className="space-y-3 text-sm">
        <div>
          <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Band Name *</label>
    ...
    if (activeSection === "salary_structures") return (
      <div className="space-y-3 text-sm">
        <div>
          <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Structure Name *</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Base %</label>
          ...
          <div>
            <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>HRA %</label>
  ...
  const renderList = () => {
    ...
    if (data.length === 0) return (
      <div className="text-center py-8">
        <div className="w-8 h-8 mx-auto mb-2 flex items-center justify-center rounded-full" style={{ background: "var(--color-light)" }}>
          {activeSection === "departments" ? <Building className="w-4 h-4" style={{ color: "var(--color-text-muted)" }} /> :
           activeSection === "designations" ? <Briefcase className="w-4 h-4" style={{ color: "var(--color-text-muted)" }} /> :
           activeSection === "bands" ? <Layers className="w-4 h-4" style={{ color: "var(--color-text-muted)" }} /> :
           <DollarSign className="w-4 h-4" style={{ color: "var(--color-text-muted)" }} />}
        </div>
        <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>No items yet</p>
      </div>
    );
    ...
    return (
      <div className="space-y-2">
        {data.map((item: any) => (
          <div key={item.id} className="flex items-center justify-between p-3 rounded-lg text-sm" style={{ background: "var(--color-light)" }}>
            <div>
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => openEdit(item)} className="p-1.5 rounded hover:bg-gray-100"><Edit2 className="w-3.5 h-3.5" style={{ color: "var(--color-primary)" }} /></button>
              <button onClick={() => handleDelete(item.id)} className="p-1.5 rounded hover:bg-gray-100"><X className="w-3.5 h-3.5" style={{ color: "var(--color-danger)" }} /></button>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-xl font-bold" style={{ color: "var(--color-navy)" }}>HR Settings</h1>
          <p className="text-sm mt-0.5" style={{ color: "var(--color-text-muted)" }}>Configure departments, designations, bands & salary structures</p>
        </div>
      </div>
        ...
        <div className="space-y-1">
          {SECTIONS.map((section) => (
            <button key={section.key} onClick={() => setActiveSection(section.key)}
              ...
              style={{ background: activeSection === section.key ? "var(--color-navy)" : "transparent", color: activeSection === section.key ? "var(--color-white)" : "var(--color-text)" }}>
