        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center rounded-lg text-xs font-medium overflow-hidden" style={{ border: "1px solid var(--color-border)" }}>
            <button
              ...
              style={{ background: activeTab === "roster" ? "var(--color-navy)" : "var(--color-light)", color: activeTab === "roster" ? "var(--color-white)" : "var(--color-text-muted)" }}
            >
            ...
            <button
              ...
              className="px-3.5 py-1.5 transition-colors flex items-center gap-1.5"
              style={{ background: activeTab === "shifts" ? "var(--color-navy)" : "var(--color-light)", color: activeTab === "shifts" ? "var(--color-white)" : "var(--color-text-muted)" }}
            >
              <Clock className="w-3.5 h-3.5" /> Shift Rotations
            </button>
          </div>

          <button onClick={() => { fetchShifts(); mutateRoster(); mutateAvail(); }} className="p-1.5 rounded-lg transition-colors" style={{ color: "var(--color-text-muted)" }} aria-label="Refresh">
        ...
        <div className="space-y-6">
          {loadingShifts ? (
            ...
            <div className="text-center py-12 border rounded-xl bg-white">
              <Clock className="w-8 h-8 mx-auto mb-2" style={{ color: "var(--color-text-muted)" }} />
              <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>No shifts configured yet</p>
              <button onClick={openAdd} className="mt-3 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-white mx-auto transition-colors" style={{ background: "var(--color-primary)" }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {shifts.map((shift) => (
                <div key={shift.id} className="rounded-xl p-5 bg-white" style={{ border: "1px solid var(--color-border)", boxShadow: "0 1px 4px rgba(var(--color-navy-rgb),0.06)" }}>
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="text-sm font-semibold" style={{ color: "var(--color-text)" }}>{shift.name}</h3>
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => openEdit(shift)} className="p-1.5 rounded hover:bg-gray-100"><Edit2 className="w-3.5 h-3.5" style={{ color: "var(--color-primary)" }} /></button>
                      <button onClick={() => handleDelete(shift.id)} className="p-1.5 rounded hover:bg-gray-100"><X className="w-3.5 h-3.5" style={{ color: "var(--color-danger)" }} /></button>
                    </div>
                  </div>
              ...
              {showForm && (
                <div className="rounded-xl p-5 bg-white" style={{ border: "2px solid var(--color-primary)", boxShadow: "0 1px 4px rgba(var(--color-navy-rgb),0.06)" }}>
                  <h3 className="text-sm font-semibold mb-3" style={{ color: "var(--color-navy)" }}>{editingId ? "Edit Shift" : "New Shift"}</h3>
                  <div className="space-y-3 text-sm">
                    <div>
                      <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Shift Name *</label>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>Start Time</label>
                        <input type="time" value={formData.start_time} onChange={(e) => setFormData({ ...formData, start_time: e.target.value })}
                          className="w-full px-3 py-1.5 rounded-lg text-xs outline-none border" style={{ borderColor: "var(--color-border)", background: "var(--color-light)" }} />
                      </div>
                      <div>
                        <label className="block text-xs font-medium mb-1" style={{ color: "var(--color-text-muted)" }}>End Time</label>
                  ...
                  <div className="flex items-center justify-end gap-2 mt-4">
                    <button onClick={() => { setShowForm(false); setEditingId(null); }} className="px-3 py-1.5 rounded-lg text-xs font-medium" style={{ color: "var(--color-text-muted)", background: "var(--color-light)" }}>Cancel</button>
