      ...
      <header className="bg-white border-b shadow-sm">
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Building2 className="w-5 h-5" style={{ color: "#1E3A8A" }} />
            <span className="text-sm font-bold" style={{ color: "#1E3A8A" }}>eHMS Direct Booking</span>
          </div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        {step === "confirm" && booking && (
          <div className="text-center space-y-6">
            <div className="inline-flex w-16 h-16 rounded-full items-center justify-center" style={{ background: "#ECFDF5" }}>
              <CheckCircle className="w-8 h-8" style={{ color: "#10B981" }} />
            </div>
            <div>
              <h1 className="text-2xl font-bold" style={{ color: "#1A3C5E" }}>Booking Confirmed!</h1>
            ...
            <Card className="text-left max-w-md mx-auto">
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium" style={{ color: "#1A3C5E" }}>{propertyCode}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span style={{ color: "#64748B" }}>Check-in</span>
                  <span className="font-medium" style={{ color: "#1A3C5E" }}>{checkIn}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span style={{ color: "#64748B" }}>Check-out</span>
                  <span className="font-medium" style={{ color: "#1A3C5E" }}>{checkOut}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span style={{ color: "#64748B" }}>Room</span>
                  <span className="font-medium" style={{ color: "#1A3C5E" }}>{selectedRoom?.unit_type}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span style={{ color: "#64748B" }}>Guest</span>
                  <span className="font-medium" style={{ color: "#1A3C5E" }}>{guestName}</span>
                </div>
                <hr style={{ borderColor: "#E2E8F0" }} />
                <div className="flex justify-between text-sm font-bold">
                  <span style={{ color: "#1A3C5E" }}>Total Paid</span>
        ...
        {step === "details" && (
          <div className="space-y-6">
            <div className="text-center mb-8">
              <h1 className="text-2xl font-bold" style={{ color: "#1A3C5E" }}>Book Your Stay</h1>
              <p className="text-sm mt-1" style={{ color: "#64748B" }}>Search availability for your dates</p>
            </div>

            <Card>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                  <label className="text-xs font-medium block mb-1" style={{ color: "#64748B" }}>Check-in</label>
                ...
                <div>
                  <label className="text-xs font-medium block mb-1" style={{ color: "#64748B" }}>Check-out</label>
                  <div className="relative">
                    <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: "#94A3B8" }} />
                    <input
                      type="date"
                ...
                <div>
                  <label className="text-xs font-medium block mb-1" style={{ color: "#64748B" }}>Guests</label>
                  <div className="relative">
                    <Users className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: "#94A3B8" }} />
                    <select
                      value={guests}
              ...
              {error && (
                <div className="mt-3 text-xs px-3 py-2 rounded-lg" style={{ background: "#FEF2F2", color: "#EF4444" }}>
                  {error}
                </div>
              )}
            </Card>
        ...
        {step === "rooms" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold" style={{ color: "#1A3C5E" }}>Available Rooms</h2>
              <p className="text-xs" style={{ color: "#94A3B8" }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {rooms.map((room) => (
                <div
                  ...
                  style={{ boxShadow: selectedRoom?.unit_type === room.unit_type ? "0 0 0 3px rgba(59,130,246,0.10)" : "0 1px 3px rgba(0,0,0,0.06)" }}
                >
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="text-sm font-bold" style={{ color: "#1A3C5E" }}>{room.unit_name || room.unit_type}</h3>
                      <p className="text-[10px] mt-0.5" style={{ color: "#94A3B8" }}>
                        Max {room.max_occupancy} guest{room.max_occupancy !== 1 ? "s" : ""}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-lg font-bold" style={{ color: "#1E3A8A" }}>{formatCurrency(room.rate_per_night)}</p>
                      <p className="text-[10px]" style={{ color: "#94A3B8" }}>per night</p>
                    </div>
                  </div>

                  <div className="flex flex-wrap gap-2 mb-3">
                    {(room.amenities || []).slice(0, 5).map((a: string) => (
                      <span key={a} className="text-[10px] px-2 py-0.5 rounded-full bg-gray-100 flex items-center gap-1" style={{ color: "#64748B" }}>
                        {AMENITY_ICONS[a] || "✨"} {a}
                      </span>
                    ))}
                  </div>

                  <div className="flex justify-between text-xs border-t pt-2" style={{ borderColor: "#E2E8F0" }}>
                    <span style={{ color: "#64748B" }}>{nights} nights total</span>
                    <span className="font-bold" style={{ color: "#1A3C5E" }}>{formatCurrency(room.rate_per_night * nights)}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {step === "guest" && selectedRoom && (
          <div className="space-y-6">
            <h2 className="text-lg font-bold" style={{ color: "#1A3C5E" }}>Guest Information</h2>

            <Card>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  <label className="text-xs font-medium block mb-1" style={{ color: "#64748B" }}>Full Name *</label>
                  <input
                    value={guestName}
                ...
                <div>
                  <label className="text-xs font-medium block mb-1" style={{ color: "#64748B" }}>Phone</label>
                  <input
                    value={guestPhone}
                ...
                <div className="md:col-span-2">
                  <label className="text-xs font-medium block mb-1" style={{ color: "#64748B" }}>Special Requests</label>
                  <textarea
            ...
            {/* Price Summary */}
            <Card>
              <h3 className="text-sm font-bold mb-3" style={{ color: "#1A3C5E" }}>Booking Summary</h3>
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span style={{ color: "#64748B" }}>{selectedRoom.unit_type} × {nights} nights</span>
                  <span style={{ color: "#1A3C5E" }}>{formatCurrency(totalBeforeDiscount)}</span>
                </div>
                {promoResult?.valid && (
                  <div className="flex justify-between text-sm">
                    <span style={{ color: "#10B981" }}>Promo discount ({promoResult.discount_pct}%)</span>
                    <span style={{ color: "#10B981" }}>-{formatCurrency(discountAmount)}</span>
                  </div>
                )}
                <hr style={{ borderColor: "#E2E8F0" }} />
                <div className="flex justify-between text-sm font-bold">
                  <span style={{ color: "#1A3C5E" }}>Total</span>
                  <span style={{ color: "#1E3A8A" }}>{formatCurrency(total)}</span>
                </div>
              </div>
            </Card>
