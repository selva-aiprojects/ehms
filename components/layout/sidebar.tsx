...
const ALL_NAV_ITEMS = [
  ...
  { label: "Subscriptions",    icon: CreditCard,     href: "/dashboard/admin/subscriptions", roles: ["platform_super_admin"] },
...
const NAV_GROUPS = [
  ...
  { label: "Administration",         icon: Shield,       items: ["Admin","Tenants","Workspaces","Roles","Audit Trail","Backup","Users","Settings","Branding","Master Data","Sessions","Compliance","Support Tickets","Broadcasts","Provisioning","Subscriptions","My Tickets"] },
...
export default function Sidebar({ mobileOpen, onMobileClose }: { mobileOpen?: boolean; onMobileClose?: () => void }) {
  ...
  return (
    <>
      ...
      {/* Mobile sidebar */}
      <aside
        className={`sidebar fixed top-0 left-0 z-50 flex flex-col transition-transform duration-300 md:hidden ${
          mobileOpen ? "translate-x-0" : "-translate-x-full"
        }`}
        style={{
          background: "var(--color-sidebar, var(--color-bg-surface))",
          width: 280,
          height: "100dvh",
        }}
      >
      ...
      {/* Desktop sidebar */}
      <aside
        className="sidebar relative flex-col transition-all duration-300 select-none hidden md:flex"
        style={{
          background: "var(--color-sidebar, var(--color-bg-surface))",
